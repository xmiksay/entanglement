//! Integration test: the read-only host trio (`read`/`glob`/`grep`) wired
//! through the real engine — a scripted LLM asks for `read`, the engine
//! dispatches it under the `build` profile (Allow), and the file content comes
//! back as a `ToolOutput`. Validates the registry wiring from ADR-0008.
