...
use entanglement_provider::{
    ...
    UserBudget, WebSearchConfig, Wire, ANTHROPIC_BASE, GEMINI_BASE, OPENAI_BASE,
};
...
fn resolve_for_user(
    ...
    // endpoint first (ADR-0147 "Consequences", ledger row 12).
...
mod tests {
    ...
    fn two_users_sharing_one_literal_key_both_resolve() {
        ...
        // isolation this produces is verified deeply in
        // `entanglement-provider`'s `client::user_budget` tests, since that's
        // where the per-user slot state lives.
        let store = InMemoryUserProviderStore::new();
        let alice = UserId::new("alice");
        let bob = UserId::new("bob");
        store.set(
