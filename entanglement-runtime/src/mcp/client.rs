//! The MCP client transport: a JSON-RPC 2.0 session over a server's stdio
//! (#198). One [`McpClient`] owns a spawned server subprocess (or, in tests, any
//! `AsyncRead`/`AsyncWrite` pair), completes the `initialize` handshake, and then
//! multiplexes `tools/list` / `tools/call` requests over the single stdin/stdout
//! pipe.
//!
...
pub struct McpClient {
    ...
    /// group-kills it when the last `Arc<McpClient>` drops).
    _child: Option<Child>,
}

impl McpClient {
    /// Spawn the configured server subprocess and complete the handshake.
    pub async fn spawn(server: &str, cfg: &McpServerConfig) -> Result<Arc<Self>> {
        ...
        let mut child = cmd
            .spawn()
            .with_context(|| format!("spawning MCP server `{server}` (`{}`)", cfg.command))?;
    ...
    pub(crate) fn new<R, W>(server: String, writer: W, reader: R, child: Option<Child>) -> Arc<Self>
    where
...
/// tool with no `name` is skipped (it can't be called).
...
mod tests {
    ...
    fn wire() -> Arc<McpClient> {
        ...
        McpClient::new("fake".to_string(), cwriter, creader, None)
    }
    ...
    async fn request_to_dead_server_errors_not_hangs() {
        ...
        let (creader, cwriter) = tokio::io::split(client_end);
        drop(_drop_server);
        let client = McpClient::new("dead".to_string(), cwriter, creader, None);
