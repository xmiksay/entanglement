...
mode: subagent
tools: [read, glob, grep, call, bash, bash_output, rhai]
permission:
  default: deny
  read: allow
  glob: allow
...
You may request shell access (e.g. `git status`, `git diff`, `git log`) via the `call`, `bash`, `bash_output`, and `rhai` tools — each such call escalates to the user for explicit approval before it runs; nothing executes silently. If you start a `bash` job with `run_in_background: true`, use `bash_output` to poll it for its result. Prefer read tools when they suffice.
