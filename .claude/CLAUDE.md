...
`opt-level = 1` for deps) lives in the workspace `../Cargo.toml`. Per-crate
`tokio` feature lists (replacing workspace-wide `features = ["full"]`), the
default-on `rhai` runtime feature, and a trimmed `syntect` feature set round
out the build-speed work (#502, [ADR-0135](../docs/adr/0135-deferred-build-speed-trims-tokio-rhai-syntect.md)).
