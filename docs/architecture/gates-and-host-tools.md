...
- **Bounded output:** 32 KiB byte cap with a truncation notice; `read` defaults
  ...
  **head-only** (`truncate_output`) — the head is what matters for a file or a
  ...
  across `bash`/`bash_output`/`call`/`rhai`/`agent`/`agent_spawn`/`agent_poll`
...
blocking `agent { agent, prompt }` (spawn-and-wait in one call) — all three
reply with `` sub-agent `id` completed in Ns:\n\n{answer} `` bounded by
