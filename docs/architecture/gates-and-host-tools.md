...
- **Approval-gated escape (ADR-0109):** containment is no longer absolute — a
  ...
  **per tool** (a `read` grant never unlocks `write`) at `Once`/`Session`/`Always`
  ...
  byte-identical to strict containment. See
