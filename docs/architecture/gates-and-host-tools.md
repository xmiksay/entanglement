...
- **Bounded output:** 32 KiB byte cap with a truncation notice; `read` defaults
  ...
  **head-only** (`truncate_output`) — the head is what matters for a file or a
  ...
  across `bash`/`call`/`rhai`/`agent`/`agent_spawn`/`poll`
...
family (§5, ADR-0033) —
`agent_spawn { agent, prompt }` (renamed from `spawn_agent`, ADR-0022) and the
blocking `agent { agent, prompt }` (spawn-and-wait in one call) — both
...
output/exit (job) or the final answer (agent) — the delta-vs-final
...
intercepted before permission resolution exactly like `agent_spawn`/`agent`
