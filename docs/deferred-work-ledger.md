...
the 2026-07-23 revisit audit) shipped and moved to Resolved. Row 5 (#517)
shipped as part of #552 and moved to Resolved. Filed against
...
| 10 ([#542](https://github.com/xmiksay/entanglement/issues/542)) | **Provider-bundled MCP enablement — three remaining scoped-out edges**: (a) a spawned child does not inherit its parent's lazy-server visibility (the `tool_spec_resolver` filter is exact-session; the child can enable it itself or the parent can use a #539 overlay); (b) `AvailableMcp` enablement marks are not cleared on `SessionEnded` (bounded memory: name → session-id set); (c) `skutter inspect` doesn't surface bundled servers (matches the #426 precedent of inspect lagging config-side MCP features). Fixed ([#555](https://github.com/xmiksay/entanglement/issues/555)): the bare-`/enable` session-tools dialog now lists each `allowed` server as an `mcp__<server>__*` row and lazy-connects it on submit, same as typed `/enable mcp <name>` and the `/mcp` panel's `e` key. Fixed ([#558](https://github.com/xmiksay/entanglement/issues/558)) for (c): `skutter inspect mcp` prints the resolved bundled/available server roster (startup-connected vs. `/enable`-gated, provider, transport, OAuth status) with no engine and no connection attempt. | [ADR-0152](adr/0152-provider-bundled-mcp-servers-three-state-enablement.md) "Consequences". | **Open** for (a)-(b); (c) closed by #558. Both remaining are UX/completeness, not correctness: an un-enabled server's tools simply don't exist for the session in question. |
