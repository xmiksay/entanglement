...
the rest of that batch has moved to the Resolved table below. Row 4 (filed by
the 2026-07-23 revisit audit) shipped and moved to Resolved. Filed against
...
| 5 ([#517](https://github.com/xmiksay/entanglement/issues/517)) | **Throttle queue depth not surfaced.** `ThrottleStatus`/`OutEvent::Throttle` report `in_flight`/`cap` (a saturated cap reads as "busy") but not how many callers are actually queued *behind* the semaphore — `tokio::sync::Semaphore` exposes no waiter count. | [ADR-0141](adr/0141-wire-visible-throttle-transitions.md) "Alternatives considered" — explicitly deferred as scope creep beyond #517's ask. | **Open.** Would need a side-counter (increment before `acquire_owned().await`, decrement after) threaded through `EndpointState`, purely for display — no request-path behavior would change. |
| 6 ([#513](https://github.com/xmiksay/entanglement/issues/513)) | **No TUI stop-cascade-vs-detach confirm modal.** The backend fully supports both (a `Stop` on the plan session detaches from a sponsored `propose_plan` build child; a second `Stop` on the child cascades), but the TUI has **no general "interrupt the in-flight turn" keybinding at all** outside the approval/`ask_user`-question-parked `Esc` paths (`Ctrl+C` is a two-stage quit, not a turn-level `Stop`) — `WaitingAgent` (the state the modal would need to key off) is neither of those. | [ADR-0145](adr/0145-one-plan-tool-file-backed-plans-and-blocking-review-loop.md) "Consequences" / "Alternatives considered". | **Open.** Needs the missing general in-turn-Stop capability built first (its own scoped piece of work), then a modal that also disambiguates `WaitingAgent`'s two callers (a plain blocking `agent` tool call vs. a sponsored `propose_plan` build) before it can safely offer the choice. |
