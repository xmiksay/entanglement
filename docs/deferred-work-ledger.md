...
Row 16 (#672) shipped 2026-08-10 for its TUI half (`is_error` now threaded
through `TranscriptEntry`/`Block::ToolCall` into `render_run.rs`'s badge) and
stays Open with a narrower remaining scope (`exit_code` only) — see
[ADR-0176](adr/0176-structured-tool-result-is-error-and-duration-fields.md)
"Consequences"; since #672 itself closed with that ship, the remainder was
re-filed as [#681](https://github.com/xmiksay/entanglement/issues/681) so the
row keeps a live issue.
Row 15 (#635) shipped 2026-08-10
([ADR-0181](adr/0181-narrate-purpose-and-per-user-aux-pins.md) amending
ADR-0154) and moved to Resolved.

Row 11 (#631) partially shipped 2026-08-10 — device-code flow (a) and the
cross-process refresh race (c) — [ADR-0182](adr/0182-mcp-oauth-device-code-flow-and-closed-refresh-race.md)
amending ADR-0153 — and stays Open with a narrower remaining scope (per-user
MCP tokens (b), OAuth for LLM provider endpoints (d)); since #631 itself
closed with that ship, the remainder was re-filed as
[#684](https://github.com/xmiksay/entanglement/issues/684) so the row keeps a
live issue.
| # | Deferred item | Documented at | Verified state |
| --- | --- | --- | --- |
| 11 ([#684](https://github.com/xmiksay/entanglement/issues/684) — orig. [#631](https://github.com/xmiksay/entanglement/issues/631), tui-ux-batch Issue 3) | **MCP OAuth — two scoped-out edges remain** (of the original four; device-code flow and the cross-process refresh race shipped in [ADR-0182](adr/0182-mcp-oauth-device-code-flow-and-closed-refresh-race.md)): (b) credentials are process-global, keyed by server name — the multi-user embedder API ([ADR-0147](adr/0147-multi-user-mode-embedder-api.md)) has per-user providers/keys/grants but *not* per-user MCP tokens; (d) OAuth is wired for MCP servers only, not for LLM provider endpoints, though the mechanism (now generic OAuth 2.1 plumbing — discovery, DCR, the authorization-code *and* device-code flows, `TokenStore`) lives in `entanglement-provider` where such a consumer would sit. | [ADR-0182](adr/0182-mcp-oauth-device-code-flow-and-closed-refresh-race.md) "Decision" (b)/(d), amending [ADR-0153](adr/0153-mcp-server-oauth.md) "Consequences". | **Open, narrower.** Neither is a correctness defect: (b) is bounded by the single-user `serve` trust model (ADR-0048), (d) is unbuilt scope. (b) direction fixed by [ADR-0181](adr/0181-userid-leaves-the-runtime-crate.md): a provider-hosted universal auth interface + `SessionId`-keyed runtime seams, re-planned in #684 on the #686/#687 base. (d) revisit trigger: a concrete OAuth-protected LLM endpoint to support. |
| 16 ([#681](https://github.com/xmiksay/entanglement/issues/681) — orig. [#672](https://github.com/xmiksay/entanglement/issues/672), orig. [#636](https://github.com/xmiksay/entanglement/issues/636)) | **`exit_code` as a real field remains out of scope.** [ADR-0176](adr/0176-structured-tool-result-is-error-and-duration-fields.md) shipped `is_error`/`duration_ms` as fields on `InMsg::ToolResult`/`OutEvent::ToolOutput` (audited against every reply site and every head) — but deliberately did *not* touch the `Tool` trait to surface `bash`/`call`'s numeric exit code as a field (still `[exit N]` text only). #672 shipped the other deferred piece — `is_error` threaded through the TUI's `TranscriptEntry`/`Block::ToolCall` rendering pipeline, so `render_run.rs`'s tool-call badge now shows the theme's red `✗` for a failed call instead of a green `✓` for any completed call regardless of outcome. | [ADR-0176](adr/0176-structured-tool-result-is-error-and-duration-fields.md) "Consequences" (the `exit_code` revisit trigger), amending [ADR-0161](adr/0161-unified-async-work-background-flag-and-one-poll.md) "Explicitly out of scope". | **Open, narrower still.** `exit_code` revisit trigger: a concrete consumer needing the numeric code, not just `[exit N]` text — still unmet. TUI half shipped 2026-08-10 (#672). |
...
| — (no issue; 2026-08-10 build-time review) | **Build-speed trims round 2 shipped.** Dependency cuts (reqwest `charset`/`encoding_rs`, the ICU4X idna backend via an `idna_adapter = "~1.1"` pin, `uuid` → direct `getrandom`, core's `async-trait` to dev-deps; Cargo.lock 347 → 328) and the 74 per-file integration-test binaries consolidated into one `tests/it` harness per crate (runtime's `rhai.rs` stays separate for its `required-features` gate). **Explicitly declined, no follow-up filed:** splitting `src/tui/` (27k lines, 38 % of the runtime crate) into its own crate — the structural cost (fourth crate, feature-graph rewiring, dep-gate/docs updates) was judged not worth the compile-time win; and `futures` → `futures-util` (two small crates for ~40 import rewrites). | [ADR-0180](adr/0180-build-speed-trims-round-2.md) amending [ADR-0135](adr/0135-deferred-build-speed-trims-tokio-rhai-syntect.md) |
| [#635](https://github.com/xmiksay/entanglement/issues/635) (orig. tui-ux-batch Issue 5) | **Aux-model deferrals shipped: a `narrate` purpose + per-user aux pins (row 15).** `Purpose` gained `Narrate` (closed-enum extension) with a new runtime-side consumer, `narrate.rs`, firing on every `OutEvent::ToolCall` (bounded to at most one in-flight aux call per session) and writing `Session.action` via `SetSessionMeta` — `action`'s first in-tree producer since [ADR-0151](adr/0151-settable-session-metadata.md). Separately, `multi_user::aux` (`UserAuxModelStore`/`InMemoryUserAuxModelStore`/`build_user_aux_registry`) mirrors `UserProviderStore` exactly, and fixes a related bug found along the way by wrapping the resolver: `resolve`/`resolve_pin` had always called the injected `ModelResolver` with a hardcoded `None` user, so a multi-user resolver would have hard-errored on every aux call regardless of the store — `AuxLlmRegistry` itself stays untouched (no `UserId` field; `entanglement-runtime`'s `skutter` binary is single-user), `build_user_aux_registry` closes the resolver over `user` instead. | [ADR-0154](adr/0154-per-purpose-auxiliary-models.md) "Consequences" / this ledger, row 15 / [ADR-0181](adr/0181-narrate-purpose-and-per-user-aux-pins.md) |
