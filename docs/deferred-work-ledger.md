...
Row 15 (#635) shipped 2026-08-12
([ADR-0183](adr/0183-narrate-purpose-and-per-user-aux-pins.md) amending
ADR-0154, conforming to [ADR-0181](adr/0181-userid-leaves-the-runtime-crate.md))
...
| [#635](https://github.com/xmiksay/entanglement/issues/635) (orig. tui-ux-batch Issue 5) | **Aux-model deferrals shipped: a `narrate` purpose + per-user aux pins (row 15).** `Purpose` gained `Narrate` (closed-enum extension) with a new runtime-side consumer, `narrate.rs`, firing on every `OutEvent::ToolCall` (bounded to at most one in-flight aux call per session) and writing `Session.action` via `SetSessionMeta` — `action`'s first in-tree producer since [ADR-0151](adr/0151-settable-session-metadata.md). Per-user pins take **no runtime module** ([ADR-0181](adr/0181-userid-leaves-the-runtime-crate.md)): the embedder builds a per-user `AuxLlmRegistry` itself from the new public `AuxModelStore::in_memory` plus a resolver closure bound to its user — the wrap also fixes `resolve`/`resolve_pin`'s hardcoded-`None` user, which would have hard-errored every aux call through a multi-user `ModelResolver`; `AuxLlmRegistry` itself carries no user notion | [ADR-0154](adr/0154-per-purpose-auxiliary-models.md) "Consequences" / this ledger, row 15 / [ADR-0183](adr/0183-narrate-purpose-and-per-user-aux-pins.md)
