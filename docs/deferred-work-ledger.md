...
| [#635](https://github.com/xmiksay/entanglement/issues/635) (orig. tui-ux-batch Issue 5) | **Aux-model deferrals shipped: a `narrate` purpose + per-user aux pins (row 15).** `Purpose` gained `Narrate` (closed-enum extension) with a new runtime-side consumer, `narrate.rs`, firing on every `OutEvent::ToolCall` (bounded to at most one in-flight aux call per session) and writing `Session.action` via `SetSessionMeta` — `action`'s first in-tree producer since [ADR-0151](adr/0151-settable-session-metadata.md). Separately, `multi_user::aux` (`UserAuxModelStore`/`InMemoryUserAuxModelStore`/`build_user_aux_registry`) mirrors `UserProviderStore` exactly, and a new `AuxLlmRegistry::for_user` fixes a related bug found along the way: `resolve`/`resolve_pin` had always called the injected `ModelResolver` with a hardcoded `None` user, so a multi-user resolver would have hard-errored on every aux call regardless of the store. | [ADR-0154](adr/0154-per-purpose-auxiliary-models.md) "Consequences" / this ledger, row 15 / [ADR-0181](adr/0181-narrate-purpose-and-per-user-aux-pins.md) |
