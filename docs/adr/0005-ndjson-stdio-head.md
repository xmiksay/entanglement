...
The stdio head lives in `entanglement-runtime` (binary `skutter`,
[ADR-0010][0010]) as two subcommands, both driving `Holly` directly (the ABI
from [ADR-0001][0001]):
...
- **`skutter pipe`** — bidirectional NDJSON relay: `InMsg` lines on stdin,
  ...
  a `Prompt` on the default session (so you can chat blind).
...
([ADR-0001][0001]), and `seq` handles ordering, not correlation.
...
- **JSON-RPC 2.0 with id correlation.** Rejected: the engine is event-stream
  oriented ([ADR-0001][0001]); request/response ids add machinery for no gain
  since `Done` already bounds a turn.
- **Length-prefixed binary framing.** Rejected: not human-readable, hostile to
  `jq`/pipes, no editor benefit over NDJSON.
- **Text-only output (no JSON mode).** Rejected: scripting needs
  machine-readable events; `--format json` is the whole point for CI/editors.
