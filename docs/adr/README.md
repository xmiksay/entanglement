...
| [0050](0050-per-endpoint-connection-pool-retry-rate-limit.md) | Provider resilience keyed **per endpoint**: RPM budget + `Retry-After` window per base URL (isolates throttling); retry classifies the response status inside the loop (fixes dead-code #193); `LlmSession` references per-endpoint state (#195) | Accepted |
